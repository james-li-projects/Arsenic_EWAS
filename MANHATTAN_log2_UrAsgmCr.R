...
snpsOfInterest_annotate <- (gwasResults %>% filter(Gene %in% FOCUS_GENE_ANNOTATE, P < 0.05/nrow(results_final)) %>% group_by(Gene) %>% filter(row_number()==1))$SNP
...
p<-ggplot(don, aes(x=BPcum, y=-log10(P))) +
  ...
  geom_text_repel(data=subset(don, is_annotate=="yes"), aes(label=Gene), size=6, point.padding = 3, direction="x", fontface="italic") +
  ...
  # outputting manhattan plot for EWAS of log2_UrAsgmCr
  png("GGPLOT2_MANHATTAN_log2_UrAsgmCr.png",units="in",height=4,width=12,res=1500)
print(p)
dev.off()
