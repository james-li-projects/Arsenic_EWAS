...
HEALS_predicted_measured_As <- data.frame(`Measured Urinary Arsenic Levels` = validation_data_cov$log2_UrAsgmCr, `Epigenetically Predicted Urinary Arsenic Scores` = estimated_log2_UrAsgmCr_validation)
colnames(HEALS_predicted_measured_As) <- c("Measured Urinary Arsenic Levels", "Epigenetically Predicted Urinary Arsenic Scores")
summary(lm(HEALS_predicted_measured_As, formula = `Measured Urinary Arsenic Levels` ~ `Epigenetically Predicted Urinary Arsenic Scores`))
png("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/EPIGENETIC_MARKER_ARSENIC_EXPOSURE/VALIDATION_CORRELATION_PLOT_HEALS.png",units="in",width=5,height=5,res=1200)
p <- ggplot(data=HEALS_predicted_measured_As, aes(x=`Epigenetically Predicted Urinary Arsenic Scores`,y=`Measured Urinary Arsenic Levels`)) + geom_point() + geom_smooth(method = "lm") + xlab("Epigenetically Predicted Urinary Arsenic Scores") + theme_classic() + theme(panel.border = element_rect(colour = "black", fill=NA),axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14)) + annotate(geom = "text", x = 0.1, y=4.78, label=expression(paste(R^2,": 0.46")),size=5) + annotate(geom = "text", x = 0.1, y=4.19, label=expression(paste("p-value: 2.73E-26")),size=5)
print(p)
dev.off()

# print out mean measured and predicted arsenic
print(paste("Mean Measured As:",mean(HEALS_predicted_measured_As$`Measured Urinary Arsenic Levels`)))
print(paste("Mean Epigenetic As:",mean(HEALS_predicted_measured_As$`Epigenetically Predicted Urinary Arsenic Scores`)))
...
plot(roc_score,cex.lab=1.25, cex.sub=1.25, xlab=substitute(paste(bold('Specificity'))),ylab=substitute(paste(bold('Sensitivity'))))
dev.off()
