...
if (exposure_var %in% c("log2_UrAsgmCr", "log2_WArsenic")) {
    ...
    save(results, file=paste0("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/results_EWAS_HEALS_combined_beta_svaSV_",exposure_var,".RData"))
    ...
    results_annotated <- merge(results, manifest, by="Name")
    results_final <- results_annotated[order(results_annotated$P.Value), ]
    output_file<-paste0("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/results_final_EWAS_HEALS_combined_beta_svaSV_",exposure_var,".RData")
    save(results_final,file=output_file)
