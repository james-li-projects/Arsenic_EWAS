...
###############################
# annotating chromosome and position for 450K
manifest<-read.csv("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/data/450k/humanmethylation450_15017482_v1-2.csv", header=T,sep=",")
...
# subsetting mval to only 450k
combined_mval <- combined_mval[rownames(combined_mval) %in% manifest$Name,]
...
save(output_cor, file=paste0("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/EPIGENETIC_MARKER_ARSENIC_EXPOSURE_450k/EPIGENETIC_MARKER_VALIDATION_R2.RData"))
...
png("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/EPIGENETIC_MARKER_ARSENIC_EXPOSURE_450k/VALIDATION_CORRELATION_PLOT_HEALS.png",units="in",width=5,height=5,res=1200)
p <- ggplot(data=HEALS_predicted_measured_As, aes(x=`Epigenetically Predicted Urinary Arsenic Levels`,y=`Measured Urinary Arsenic Levels`)) + geom_point() + geom_smooth(method = "lm") + xlab("Epigenetically Predicted Urinary Arsenic Scores") + theme_classic() + theme(panel.border = element_rect(colour = "black", fill=NA),axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14)) + annotate(geom = "text", x = 0.70, y=4.9, label=expression(paste(R^2,": 0.34")),size=5) + annotate(geom = "text", x = 0.70, y=4.4, label=expression(paste("p-value: 4.83E-18")),size=5)
print(p)
dev.off()
...
save(EPIGENETIC_MARKER_DF, file=paste0("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/EPIGENETIC_MARKER_ARSENIC_EXPOSURE_450k/EPIGENETIC_MARKER_DF.RData"))
...
print(paste("Unadjusted AUC of Baseline Skin Lesion Status:", toString(sort(as.numeric(ci(output_auc))))))
save(output_auc, file=paste0("/gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/EPIGENETIC_MARKER_ARSENIC_EXPOSURE_450k/EPIGENETIC_MARKER_VALIDATION_SL_AUC.RData"))
...
output_AROC.sp <- AROC.sp(
  ...
  tag.h = 0)
print(paste("Covariate-adjusted AUC of Baseline Skin Lesion Status:", toString(sort(as.numeric(output_AROC.sp$AUC)))))
...
png(paste0("EPIGENETIC_MARKER_ROC_SL.png"),units="in",width=5,height=5,res=1200)
plot(roc_score)
dev.off()
...
output_string_command = paste0("echo ",output_string," >> /gpfs/data/phs/groups/Projects/GEMS/james.li/EWAS/manuscript/output/EPIGENETIC_MARKER_ARSENIC_EXPOSURE_450k/output.log")
system(output_string_command)

# performing survival analysis
library(survival)
follow_up_data<-fread("/gpfs/data/phs/groups/Projects/HEALS datasets/ACC/followup_time_updated_06272023.txt") %>% mutate(Sample_Name = paste("X",SubjectID,sep=""), year_FU=`time(days)`/365.25) %>% filter(!is.na(`time(days)`))
follow_up_data$dead[follow_up_data$va==1] <- 1
follow_up_data$dead[is.na(follow_up_data$va)] <- 0
follow_up_data <- follow_up_data %>% select(Sample_Name,year_FU,dead)
# joining covariates and follow up data
joined_df <- inner_join(validation_data_cov, follow_up_data, by=c("Sample_Name"))
joined_df$year_FU <- joined_df$year_FU + joined_df$Age

# overall mortality - predicted arsenic 
coxph(formula = Surv(Age,year_FU,dead) ~ SCORE + factor(Sex) + factor(bmi_cat) + factor(cigsmoke), data = joined_df)
